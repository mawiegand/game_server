class Action::Settlement::MoveSettlementToRegionActionsController < ApplicationController
  ...
  def create
    ...
    raise NotFoundError.new('region not existing') if region.nil?
    raise ForbiddenError.new("already exists one settlement in region") unless region.settleable_by?(current_character)
    raise BadRequestError.new("moving is not allowed") unless region.is_moving_allowed?(current_character.alliance, params[:move_settlement_action][:region_password])
    ...
    raise ConflictError.new("home_base is fighting") if home_base.fighting?
