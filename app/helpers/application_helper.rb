module ApplicationHelper
  ...
  def render_not_modified_or(last_modified)
    ...
    logger.debug("MODIFIED COMPARISION: #{ last_modified <= Time.httpdate(if_modified_since) } = #{last_modified} <= #{ Time.httpdate(if_modified_since) }.")   if last_modified && if_modified_since
