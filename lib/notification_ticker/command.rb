...
module NotificationTicker
  class Command
    ...
    def daemonize
      ...
      process_name = "#{File.basename($0)}.#{@options[:identifier]}"
      run_process(process_name, dir)
    end

    def run_process(process_name, dir)
      NotificationTicker::Runloop.before_fork      
    ...
    def run(worker_name = nil)
      ...
      logname = "notification_ticker_#{Rails.env}.log"

      NotificationTicker::Runloop.after_fork      
      NotificationTicker::Runloop.logger ||= Logger.new(File.join(Rails.root, 'log', logname))

      runloop = NotificationTicker::Runloop.new(@options)
      runloop.start
    rescue => e
