...
module NpcAi
  class Command
    ...
    def run(worker_name = nil)
      ...
      logname = "npc_ai_#{Rails.env}.log"

      NpcAi::Runloop.after_fork      
      NpcAi::Runloop.logger ||= Logger.new(File.join(Rails.root, 'log', logname))

      runloop = NpcAi::Runloop.new(@options)
      runloop.start
    rescue => e
